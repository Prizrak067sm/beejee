<?php
// Примем следующее соглашение - uri состоит из: точки входа / контроллера / метода контроллера
// Например: domen/index/main/default  -  index=точка входа; main=контроллер; default=метода контроллера
    class Route
    {
        static $URI;
        static $dir_app;
        ...
        static function start()
        {
            ...
            Route::$file_entry_point = $routes[1];
            Route::$dir_app = 'app_'.Route::$file_entry_point;

            // Получаем имя контроллера из запроса.
            if (!empty($routes[2]))
            {
                $controller_name = $routes[2];   // Имя контроллера из запроса.
            }
            // ------------------------------------
            ...
            if (!empty($routes[3]))
            {
                $action_name = $routes[3];   // Имя действия (экшена) из запроса.
            }
            // --------------------------------------
        ...
        static function error_page404()
        {
            ...
            header("Location: /".Route::$file_entry_point."/page404");
